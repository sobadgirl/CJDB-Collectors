...
class FunASRProvider(BaseDataProvider, VideoTranscriptionProviderMixin):
    ...
    def _setup(self) -> SetupResult:
        ...
        self._log("FunASR setup 完成")
        for line in logs:
            self.logger.info(line)
        return SetupResult(
            success=True,
            message="FunASR setup 完成",
            setup_payload={**self.setup_payload, "model_paths": normalized_paths},
        )
        ...
        for entry in entries:
            ...
            if isinstance(sentence_info, list):
                segment_texts.extend(
                    ...
                    for segment in self._normalize_segments(sentence_info)
                )

        normalized_text = "".join(texts).strip()
        if not normalized_text and segment_texts:
            normalized_text = "".join(segment_texts).strip()
        return TranscriptionResult(text=normalized_text)
