...
def test_config_yaml_secrets_are_mapped_and_masked(tmp_path) -> None:
    ...
    assert isinstance(settings.services.tikhub.api_key, SecretStr)
    assert settings.services.tikhub.api_key.get_secret_value() == "tikhub-secret"
    assert settings.services.collector.api_key.get_secret_value() == "collector-secret"
    assert settings.secrets.resolve("notion_token") == "notion-secret"
    assert settings.secrets.resolve("notion-token") == "notion-secret"
    dumped = settings.model_dump(mode="json")
    assert dumped["secrets"]["notion_token"] == "**********"
